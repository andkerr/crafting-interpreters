...
static uint8_t makeConstant(Value value) {
    // TODO: make addConstant return the index of duplicate constants, rather
    // than always inserting a new one
    int constant = addConstant(currentChunk(), value);
    if (constant > UINT8_MAX) {
        error("Maximum constants exceeded in chunk");
        return 0;
    }
...
static void dot(bool canAssign) {
    consume(TOKEN_IDENTIFIER, "Execpted property name after '.'");
...
static void literal(bool canAssign) {
    ...
    switch (parser.previous.type) {
        ...
        default: error("literal: unreachable"); exit(1);
    }
}
...
static void funDeclaration() {
    ...
    markInitialized(); // allow self-references when compiling local functions
...
static void switchStatement() {
    ...
    expression();
    int switchLocalIdx = current->localCount++;
...
static void block() {
    ...
    consume(TOKEN_RIGHT_BRACE, "Expected closing '}' to match '{'");
}
...
static void function(FunctionType type) {
    ...
    emitBytes(OP_CLOSURE, makeConstant(OBJ_VAL(function))); // scope ends...
