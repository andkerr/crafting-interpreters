...
static int constantInstruction(const char *name, const Chunk *chunk,
                               int offset, bool constant_long) {
    int i = 0;
    if (!constant_long) {
        i = chunk->code[offset + 1];
        offset += 2;
    }
    else {
        i |= (chunk->code[offset + 3] << 16);
        i |= (chunk->code[offset + 2] <<  8);
        i |= (chunk->code[offset + 1] <<  0);
        offset += 4;
    }
...
int disassembleInstruction(const Chunk *chunk, int offset) {
    ...
    switch (instruction) {
        ...
        case OP_CLOSURE: {
            ...
            for (int _ = 0; _ < function->upvalueCount; ++_) {
